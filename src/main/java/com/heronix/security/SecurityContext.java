 ...
 *
 * Location: src/main/java/com/heronix/security/SecurityContext.java
 *
...
public class SecurityContext {
    ...
    /**
     * Get current user's staff ID (User ID serves as staff ID)
     * Returns 1L as default for system operations if no user is authenticated
     *
     * @return Staff ID (User ID) or 1L if not authenticated
     */
    public static Long getCurrentStaffId() {
        return getCurrentUser()
                .map(User::getId)
                .orElse(1L); // Default system user ID
    }

    /**
     * Get current user ID
     * Same as getCurrentStaffId() but throws exception if not authenticated
     *
     * @return User ID
