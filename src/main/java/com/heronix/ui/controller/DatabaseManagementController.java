...
public class DatabaseManagementController {
    ...
    private void handleViewStats() {
        try {
            ...
            try (Connection conn = dataSource.getConnection()) {
                ...
                // Database Info
                stats.append("DATABASE INFORMATION:\n");
                stats.append("Database Product: ").append(metaData.getDatabaseProductName()).append("\n");
                stats.append("Database Version: ").append(metaData.getDatabaseProductVersion()).append("\n");
                stats.append("Driver Name: ").append(metaData.getDriverName()).append("\n");
                stats.append("Driver Version: ").append(metaData.getDriverVersion()).append("\n");
                stats.append("URL: ").append(metaData.getURL()).append("\n\n");
                ...
                while (tables.next()) {
                    ...
                    if (!tableName.startsWith("FLYWAY_") && !tableName.startsWith("DATABASECHANGELOG")) {
                        tableCount++;
                        ...
                        try (var stmt = conn.createStatement();
                            ...
                            if (rs.next()) {
                                ...
                                totalRows += rowCount;
                                stats.append(String.format("  %-40s %,10d rows\n", tableName, rowCount));
                            }
                        } catch (Exception e) {
                            stats.append(String.format("  %-40s %s\n", tableName, "Error reading"));
                        }
                    }
                }
                tables.close();
                ...
                stats.append("Total Rows: ").append(String.format("%,d", totalRows)).append("\n\n");

                // Index Statistics
                stats.append("INDEX INFORMATION:\n");
                int indexCount = 0;
                ...
                stats.append("Total Indexes: ").append(indexCount).append("\n\n");

                // Performance Metrics
                stats.append("PERFORMANCE METRICS:\n");
                stats.append("Connection Pool Active: ").append(conn.isClosed() ? "No" : "Yes").append("\n");
                stats.append("Auto Commit: ").append(conn.getAutoCommit() ? "Enabled" : "Disabled").append("\n");
                stats.append("Transaction Isolation: ");
                switch (conn.getTransactionIsolation()) {
                    case Connection.TRANSACTION_READ_UNCOMMITTED:
                        stats.append("READ_UNCOMMITTED\n");
                        break;
                    case Connection.TRANSACTION_READ_COMMITTED:
                        stats.append("READ_COMMITTED\n");
                        break;
                    case Connection.TRANSACTION_REPEATABLE_READ:
                        stats.append("REPEATABLE_READ\n");
                        break;
                    case Connection.TRANSACTION_SERIALIZABLE:
                        stats.append("SERIALIZABLE\n");
                        break;
                    default:
            ...
            Platform.runLater(() -> {
                ...
                alert.setTitle("Database Statistics");
            ...
            log.error("Error generating database statistics", e);
            showError("Error", "Failed to generate statistics: " + e.getMessage());
        }
    }
    ...
    private void handleAnalyzeTables() {
        try {
            ...
            analysis.append("TABLE ANALYSIS REPORT\n");
            analysis.append("=====================\n\n");
            ...
            try (Connection conn = dataSource.getConnection()) {
                ...
                while (tables.next()) {
                    ...
                    analyzedCount++;
                    analysis.append("TABLE: ").append(tableName).append("\n");
                    analysis.append("─".repeat(60)).append("\n");
                    ...
                    while (columns.next()) {
                        ...
                        analysis.append(String.format("  Column: %-30s Type: %-15s Size: %-6d Nullable: %s\n",
                    ...
                    while (indexes.next()) {
                        ...
                        if (indexName != null) {
                            ...
                            analysis.append(String.format("  Index: %-30s Column: %-20s Unique: %s\n",
                                indexName, columnName, !nonUnique ? "Yes" : "No"));
                        }
                    }
                    indexes.close();
                    ...
                    while (primaryKeys.next()) {
                        ...
                        analysis.append(String.format("  Primary Key: %-30s Column: %s\n", pkName, columnName));
                    }
                    primaryKeys.close();
                    ...
                    try (var stmt = conn.createStatement();
                        ...
                        if (rs.next()) {
                            long rowCount = rs.getLong(1);
                            analysis.append(String.format("\n  Summary: %d columns, %d indexes, %,d rows\n",
                                columnCount, indexCount, rowCount));
                        }
                    }
                ...
                analysis.append("\nTOTAL TABLES ANALYZED: ").append(analyzedCount).append("\n");

                // Optimization suggestions
                analysis.append("\nOPTIMIZATION SUGGESTIONS:\n");
                analysis.append("• Consider adding indexes to frequently queried columns\n");
                analysis.append("• Remove unused indexes to improve write performance\n");
                analysis.append("• Regularly vacuum/analyze database for optimal performance\n");
                analysis.append("• Monitor tables with high row counts for partitioning opportunities\n");
            }

            // Show analysis in dialog
            Platform.runLater(() -> {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Table Analysis");
                alert.setHeaderText(null);

                TextArea textArea = new TextArea(analysis.toString());
                textArea.setEditable(false);
                textArea.setWrapText(false);
                textArea.setPrefRowCount(30);
            ...
            showError("Error", "Failed to analyze tables: " + e.getMessage());
        }
    }
    ...
    private void handleVacuumDatabase() {
        ...
        confirm.setTitle("Vacuum Database");
        ...
        confirm.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    progress.setTitle("Database Optimization");
                    progress.setHeaderText("Optimizing database...");

                    TextArea progressArea = new TextArea();
                    progressArea.setEditable(false);
                    progressArea.setPrefRowCount(15);
                    progress.getDialogPane().setContent(progressArea);
                    progress.show();
                    ...
                    CompletableFuture.runAsync(() -> {
                        try (Connection conn = dataSource.getConnection()) {
                            ...
                            Platform.runLater(() -> progressArea.appendText("Database: " + dbProduct + "\n"));

                            if (dbProduct.contains("h2")) {
                                // H2 Database optimization
                                Platform.runLater(() -> progressArea.appendText("Running H2 optimization...\n"));

                                try (var stmt = conn.createStatement()) {
                                    // Analyze all tables
                                    Platform.runLater(() -> progressArea.appendText("Analyzing tables...\n"));
                                    stmt.execute("ANALYZE");

                                    // Compact database
                                    Platform.runLater(() -> progressArea.appendText("Compacting database...\n"));
                                    stmt.execute("SHUTDOWN COMPACT");
                                }

                            } else if (dbProduct.contains("postgresql")) {
                                // PostgreSQL vacuum
                                Platform.runLater(() -> progressArea.appendText("Running PostgreSQL VACUUM...\n"));
                            ...
                            Platform.runLater(() -> {
                                ...
                                progressArea.appendText("Performance improvements applied.\n");
                            ...
                            log.error("Error during vacuum operation", e);
                            Platform.runLater(() -> {
                                progress.close();
                                showError("Vacuum Error", "Failed to optimize database: " + e.getMessage());
                            });
                        }
                    });

                } catch (Exception e) {
                    log.error("Error starting vacuum operation", e);
                    showError("Error", "Failed to start vacuum: " + e.getMessage());
                }
            }
        });
    }
    ...
    private void handleResetDatabase() {
        Alert warning = new Alert(Alert.AlertType.WARNING);
        warning.setTitle("⚠️ Reset Database");
        warning.setHeaderText("Reset Database to Default State");
        warning.setContentText("This will:\n" +
            "• Keep the database structure (tables, indexes)\n" +
            "• Delete all data from all tables\n" +
            "• Recreate essential reference data\n" +
        ...
        warning.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK && "RESET DATABASE".equals(confirmField.getText())) {
                try {
                    log.warn("⚠️ User confirmed database reset");

                    Alert progress = new Alert(Alert.AlertType.INFORMATION);
                    progress.setTitle("Resetting Database");
                    progress.setHeaderText("Resetting database to default state...");
                    TextArea progressArea = new TextArea();
                    progressArea.setEditable(false);
                    progressArea.setPrefRowCount(10);
                    progress.getDialogPane().setContent(progressArea);
                    ...
                    // Execute reset in background
                    CompletableFuture.runAsync(() -> {
                        try {
                            // Use purge service to clear data
                            purgeDatabaseService.purgeAllData(message -> {
                                Platform.runLater(() -> progressArea.appendText(message + "\n"));
                            });

                            Platform.runLater(() -> {
                                progressArea.appendText("\n✅ Database reset completed!\n");
                                progressArea.appendText("Database is now ready for use.\n");
                            ...
                            Platform.runLater(() -> {
                                progress.close();
                                CopyableErrorDialog.showError("Reset Failed",
                    ...
                    log.error("Error initiating database reset", e);
                    showError("Reset Error", "Failed to start database reset: " + e.getMessage());
                }
            }
        });
    }
    ...
    private void handleExportSchema() {
        try {
            FileChooser fileChooser = new FileChooser();
            fileChooser.setTitle("Export Database Schema");
            fileChooser.setInitialFileName("database_schema_" +
                LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd_HHmmss")) + ".sql");
            fileChooser.getExtensionFilters().add(
                new FileChooser.ExtensionFilter("SQL Files", "*.sql"));

            File outputFile = fileChooser.showSaveDialog(null);
            if (outputFile != null) {
                log.info("Exporting database schema to: {}", outputFile);

                StringBuilder schema = new StringBuilder();
                schema.append("-- Database Schema Export\n");
                schema.append("-- Generated: ").append(LocalDateTime.now()).append("\n");
                schema.append("-- ========================================\n\n");

                try (Connection conn = dataSource.getConnection()) {
                    DatabaseMetaData metaData = conn.getMetaData();

                    schema.append("-- Database: ").append(metaData.getDatabaseProductName())
                    ...
                    while (tables.next()) {
                        ...
                        while (columns.next()) {
                            ...
                            schema.append("  \"").append(columnName).append("\" ")
                                .append(dataType);

                            if (size > 0 && !dataType.contains("TEXT") && !dataType.contains("BLOB")) {
                                schema.append("(").append(size).append(")");
                            }
                        ...
                        while (indexes.next()) {
                            ...
                            if (indexName != null && !indexName.startsWith("PRIMARY")) {
                                ...
                                schema.append("CREATE ");
                                if (!nonUnique) {
                                    schema.append("UNIQUE ");
                                }
                                schema.append("INDEX IF NOT EXISTS \"").append(indexName)
                ...
                // Write to file
                Files.writeString(outputFile.toPath(), schema.toString());

                showInfo("Export Complete", "Database schema exported successfully to:\n" + outputFile.getAbsolutePath());
                log.info("✅ Schema exported successfully");

            }
        } catch (Exception e) {
            log.error("Error exporting schema", e);
            showError("Export Error", "Failed to export schema: " + e.getMessage());
        }
    }
    ...
    private void handleImportSQL() {
        ...
        if (sqlFile != null) {
            Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
            confirm.setTitle("Import SQL File");
            confirm.setHeaderText("Execute SQL from file?");
            confirm.setContentText("File: " + sqlFile.getName() + "\n\n" +
                "This will execute all SQL statements in the file.\n" +
            ...
            confirm.showAndWait().ifPresent(response -> {
                if (response == ButtonType.OK) {
                    try {
                        log.info("📥 Importing SQL from: {}", sqlFile);

                        Alert progress = new Alert(Alert.AlertType.INFORMATION);
                        progress.setTitle("Importing SQL");
                        progress.setHeaderText("Executing SQL statements...");
                        TextArea progressArea = new TextArea();
                        progressArea.setEditable(false);
                        progressArea.setPrefRowCount(15);
                        progress.getDialogPane().setContent(progressArea);
                        ...
                        CompletableFuture.runAsync(() -> {
                            try {
                                ...
                                Platform.runLater(() ->
                                    progressArea.appendText("Found " + statements.length + " SQL statements\n\n"));
                                ...
                                try (Connection conn = dataSource.getConnection();
                                    ...
                                    for (String sql : statements) {
                                        ...
                                        try {
                                            ...
                                            final int count = executed;
                                            Platform.runLater(() ->
                                                progressArea.appendText("✓ Executed statement " + count + "\n"));

                                        } catch (Exception e) {
                                            failed++;
                                            final String error = e.getMessage();
                                            Platform.runLater(() ->
                                                progressArea.appendText("✗ Error: " + error + "\n"));
                                ...
                                Platform.runLater(() -> {
                                    progressArea.appendText("\n" + "=".repeat(50) + "\n");
                                    progressArea.appendText("Import completed!\n");
                                    progressArea.appendText("Executed: " + finalExecuted + " statements\n");
                                    progressArea.appendText("Failed: " + finalFailed + " statements\n");
                                ...
                                log.info("✅ SQL import completed: {} executed, {} failed", finalExecuted, finalFailed);

                            } catch (Exception e) {
                                log.error("❌ SQL import failed", e);
                                Platform.runLater(() -> {
                                    progress.close();
                                    showError("Import Failed", "Failed to import SQL: " + e.getMessage());
                                });
                            }
                        });

                    } catch (Exception e) {
                        log.error("Error starting SQL import", e);
                        showError("Import Error", "Failed to start import: " + e.getMessage());
                    }
                }
            });
        }
