...
public interface EmergencyContactRepository extends JpaRepository<EmergencyContact, Long> {
    ...
    /**
     * Find all emergency contacts for a student (active and inactive), ordered by priority
     */
    @Query("SELECT ec FROM EmergencyContact ec WHERE ec.student.id = :studentId " +
           "ORDER BY ec.priorityOrder ASC")
    List<EmergencyContact> findByStudentIdOrderByPriorityOrder(@Param("studentId") Long studentId);
    ...
    /**
     * Find emergency contacts by student ID and active status, ordered by priority
     *
     * @param studentId The student ID
     ...
     */
    @Query("SELECT ec FROM EmergencyContact ec WHERE ec.student.id = :studentId " +
           "AND ec.isActive = :isActive " +
           "ORDER BY ec.priorityOrder ASC")
    List<EmergencyContact> findByStudentIdAndIsActiveOrderByPriorityOrder(
            @Param("studentId") Long studentId, @Param("isActive") boolean isActive);

    /**
     * Find emergency contacts authorized for pickup by student ID
     *
     * @param studentId The student ID
     * @param authorizedToPickUp Authorized to pick up
     * @param isActive Active status
     * @return List of emergency contacts ordered by priority
     */
    @Query("SELECT ec FROM EmergencyContact ec WHERE ec.student.id = :studentId " +
           "AND ec.authorizedToPickUp = :authorizedToPickUp " +
           "AND ec.isActive = :isActive " +
           "ORDER BY ec.priorityOrder ASC")
    List<EmergencyContact> findByStudentIdAndAuthorizedToPickUpAndIsActiveOrderByPriorityOrder(
            @Param("studentId") Long studentId,
            @Param("authorizedToPickUp") boolean authorizedToPickUp,
            @Param("isActive") boolean isActive);

    /**
     * Find all emergency contacts (including inactive)
     *
     ...
     */
    @Query("SELECT ec FROM EmergencyContact ec WHERE " +
           "LOWER(ec.relationship) = LOWER(:relationship) " +
           "AND ec.isActive = true")
    List<EmergencyContact> findByRelationship(@Param("relationship") String relationship);

    /**
     * Find contacts by student ID and relationship
     *
    ...
    @Query("SELECT ec FROM EmergencyContact ec WHERE ec.student.id = :studentId " +
           ...
           "AND ec.isActive = true " +
           "ORDER BY ec.priorityOrder ASC")
    List<EmergencyContact> findByStudentIdAndRelationship(
            @Param("studentId") Long studentId, @Param("relationship") String relationship);
     ...
     */
    @Query("SELECT ec FROM EmergencyContact ec WHERE " +
           "(ec.verificationDate IS NULL OR ec.verified = false) " +
           "AND ec.isActive = true")
    List<EmergencyContact> findUnverified();

    /**
     * Find contacts missing phone numbers
     *
     * @return List of contacts without phone
     */
    @Query("SELECT ec FROM EmergencyContact ec WHERE " +
           "ec.primaryPhone IS NULL AND ec.secondaryPhone IS NULL AND ec.workPhone IS NULL " +
           "AND ec.isActive = true")
    List<EmergencyContact> findMissingPhone();

    /**
     * Find incomplete contacts
     *
     * @return List of incomplete contacts
     */
    @Query("SELECT ec FROM EmergencyContact ec WHERE ec.isActive = true AND (" +
           "ec.firstName IS NULL OR ec.lastName IS NULL OR " +
           "ec.relationship IS NULL OR " +
           "(ec.primaryPhone IS NULL AND ec.secondaryPhone IS NULL AND ec.workPhone IS NULL))")
    List<EmergencyContact> findIncomplete();
}
